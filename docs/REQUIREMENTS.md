...
Автор и разработчик — akiamuradev. Основной режим эксплуатации — собственный сервер
в локальной сети.
...
- `title`: отображаемое имя до 160 символов; для draft может быть пустым, для проверки и
  публикации обязательно 2–160 символов;
...
- `summary`: краткое описание до 500 символов; для draft может быть пустым, для проверки и
  публикации обязательно 20–500 символов;
- `description`: Markdown до 30 000 символов; для draft может быть пустым, raw HTML запрещён;
...
REQ-CARD-008. Ручную карточку можно сохранить как неполный draft без title, summary и
description; при пустом slug сервер создаёт уникальный стабильный временный адрес. До первого
сохранения editor может загрузить до 12 изображений в принадлежащую ему staging-область,
просматривать их и менять описания. При создании draft перечисленные asset UUID проверяются по
owner, типу и состоянию и прикрепляются к новой карточке в той же транзакции. Неиспользованные
staging uploads очищаются общей retention-процедурой. Требования полноты применяются при
отправке на проверку и публикации, но не мешают промежуточному сохранению.
...
REQ-UI-010. Основные страницы доступны с клавиатуры и сохраняют видимый фокус. Поля формы
имеют доступные подписи, повторяющиеся действия — однозначные названия, а вкладки редактора
поддерживают Arrow Left/Right, Home и End. На ширине 320 px интерфейс не создаёт горизонтальную
прокрутку; светлая и тёмная темы проходят автоматическую проверку WCAG 2 AA средствами axe.
...
- REQ-NFR-009: сервер работает на Ubuntu Server со static IP и exact internal
...
1. Получить письменное решение правообладателя источника по объёму копирования, хранению
