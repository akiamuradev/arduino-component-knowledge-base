...
Есть regression tests смены пользователя, очистки кэша при logout/401, позднего
ответа отменённого запроса и сохранения сессии при 403. Браузерный сценарий проверяет
login/logout, а accessibility-проверки — контраст OLED в обеих темах на узком экране.

PostgreSQL integration проверяет изоляцию shadow pipeline: SQL failure, ORM flush
failure и логический отказ после persistence откатывают только shadow savepoint.
Записи внешней транзакции сохраняются, последующие записи разрешены; успешный shadow
не коммитит внешнюю транзакцию самостоятельно.

Query-budget regression на PostgreSQL защищает пакетную сборку каталога: public page
из трёх карточек с media использует 6 SQL-запросов, workspace page — 10. Отдельно
проверяются порядок примеров и загрузка всех hints одним join-запросом.
...
Каталог проверяется при масштабе 100% на 1366×768, 1440×900, 1920×1080 и 2560×1440,
а также на границе desktop layout, планшете и мобильных ширинах 320–360 px.
Регрессии проверяют шесть колонок на Full HD, реальный `total` API (включая ноль),
sticky-фильтры, сброс, переход с клавиатуры к поиску, отсутствие горизонтального
переполнения и контраст обеих тем. Для SVG-платы проверяются ограниченный наклон,
объединение pointer events в один кадр и статическое состояние при reduced motion.
Desktop-скриншоты в `docs/screenshots` используют фиксированные тестовые карточки,
а не статистику или содержимое рабочей базы.
