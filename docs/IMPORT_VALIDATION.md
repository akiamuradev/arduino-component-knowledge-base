...
Инструкция сохранена для изолированной проверки adapters Seeed Studio Wiki и Official KiCad
Symbols. В штатной конфигурации оба источника неактивны, поэтому API отклоняет новые jobs с
`source_disabled`; выполнять сценарий можно только на отдельном временном стенде с явно
изменённой source policy. Массовая загрузка и автоматическая публикация не разрешены.
