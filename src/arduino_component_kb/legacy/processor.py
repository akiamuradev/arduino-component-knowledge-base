...
async def apply_item(
    ...
    if previous is not None or item.decision == "skip":
        ...
        item.status = "skipped"
        await audit(session, bundle.owner_id, "item_skipped", item.id)
        return
    ...
    if item.decision == "merge" and item.merge_id is not None:
        ...
        card = await service.update(card.id, card.revision, merged, bundle.owner_id)
        action = "item_merged"
    elif item.decision == "create":
        card = await service.create(incoming, bundle.owner_id)
        action = "item_created"
    else:
    ...
    await service.touch_media_attachment(card.id, card.revision, bundle.owner_id, datetime.now(UTC))
    item.result_id = card.id
    item.status = "applied"
    item.error_code = None
    await audit(session, bundle.owner_id, action, item.id)
