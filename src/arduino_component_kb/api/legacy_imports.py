...
async def create_bundle(
    body: BundleInput, request: Request, session: Session, principal: Admin, csrf: CSRF
) -> dict[str, object]:
...
@router.get("")
async def list_bundles(session: Session, principal: Admin) -> list[dict[str, object]]:
...
async def get_bundle(bundle_id: UUID, session: Session, principal: Admin) -> dict[str, object]:
...
async def complete_upload(
    bundle_id: UUID, request: Request, session: Session, principal: Admin, csrf: CSRF
) -> dict[str, str]:
...
async def start_analysis(
    bundle_id: UUID, session: Session, principal: Admin, csrf: CSRF
) -> dict[str, str]:
    bundle = await locked_bundle(session, bundle_id)
...
async def apply_bundle(
    bundle_id: UUID, body: ApplyInput, session: Session, principal: Admin, csrf: CSRF
) -> dict[str, str]:
    bundle = await locked_bundle(session, bundle_id)
    items = await bundle_items(session, bundle_id)
    if bundle.status != "ready" or review_hash(bundle, items) != body.plan_hash:
...
async def review_license(
    component_id: UUID, body: LicenseInput, session: Session, principal: Admin, csrf: CSRF
) -> dict[str, str]:
    rows = list(
        await session.scalars(
